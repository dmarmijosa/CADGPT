...
test('JWT signature, audience, expiry and scopes are mandatory', async () => {
  ...
  const server = createServer((_q, r) => { r.setHeader('content-type', 'application/json'); r.end(JSON.stringify({ keys: [{ ...jwk, kid: 'test', alg: 'RS256' }] })); });
  await new Promise<void>(r => server.listen(0, '127.0.0.1', r));
  try {
    const address = server.address();
    assert.ok(address && typeof address !== 'string');
    const verify = authenticator('https://issuer.example', 'cadgpt-api', 'http://127.0.0.1:' + address.port);
    const sign = (aud: string, exp: number, scope: string) => new SignJWT({ scope }).setProtectedHeader({ alg: 'RS256', kid: 'test' }).setSubject('alice').setIssuer('https://issuer.example').setAudience(aud).setExpirationTime(exp).sign(keys.privateKey);
